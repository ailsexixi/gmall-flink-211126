...
public class DwdTradeOrderRefund {
    public static void main(String[] args) throws Exception {
        ...
        // 为表关联时状态中存储的数据设置过期时间
        configuration.setString("table.exec.state.ttl", "5 s");// =tableEnv.getConfig().setIdleStateRetention(Duration.ofSeconds(5));
...
//        ));
//        env.setStateBackend(new HashMapStateBackend()); //设置本地数据状态储存在内存
//        env.getCheckpointConfig().setCheckpointStorage(  //设置checkpoint状态储存在文件中
...
//        System.setProperty("HADOOP_USER_NAME", "atguigu");
        ...
        tableEnv.executeSql("create table topic_db(" +
                ...
                "`ts` string " +
                ")" + MyKafkaUtil.getKafkaDDL("topic_db", "order_refund_211126"));
